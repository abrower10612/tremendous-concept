...
import { X, Trash2, Upload, Settings, CornerDownLeft } from 'lucide-react'
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="mx-auto max-w-5xl px-8 py-10">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold text-ink">Recipients</h1>
          <div className="flex items-center gap-3">
            ...
            <button className="rounded-md border border-line p-2 hover:bg-gray-50">
        ...
        <div className="mt-6 overflow-hidden rounded-xl border border-line">
          <div className="flex items-center justify-between border-b border-line bg-gray-50 px-4 py-2.5 text-sm">
            <span>
            ...
            {draft.recipients.length > 0 && (
              <button
                onClick={() => draft.recipients.forEach((r) => draft.removeRecipient(r._key))}
          ...
          {/* Column headers */}
          <div className="grid grid-cols-[1fr_1.4fr_140px_140px_56px] gap-3 border-b border-line px-4 py-2 text-xs font-medium text-muted">
          ...
          {/* Manual add row */}
          <div className="grid grid-cols-[1fr_1.4fr_140px_140px_56px] items-center gap-3 border-b border-line px-4 py-3">
            <input
              value={name}
            ...
            <button
              onClick={addManual}
              className="flex items-center justify-center gap-1 rounded-md border border-brand px-3 py-2 text-sm font-medium text-brand hover:bg-brand/5"
            >
          ...
          {draft.recipients.map((r) => (
            <div
              key={r._key}
              className="grid grid-cols-[1fr_1.4fr_140px_140px_56px] items-center gap-3 border-b border-line px-4 py-3 text-sm last:border-b-0"
            >
        ...
        {draft.recipients.length > 0 && (
          <button
            onClick={() => navigate('/order/new')}
            className="mt-8 rounded-md bg-brand px-6 py-2.5 text-sm font-semibold text-white hover:bg-brand-hover"
          >
            Continue
          </button>
        )}
